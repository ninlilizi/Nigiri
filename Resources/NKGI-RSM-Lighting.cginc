...
float4 RSMFluxLight(float2 uv, float3 pos, float3 worldNormal, UnityLight light) {

	float3 lightColor = light.color;
	float3 diffuse = tex2D(_MainTex, uv).rgb;
	float3 lightDir = normalize(light.dir - pos.xyz);
	float4 normal = float4(worldNormal, 1.0);

	return float4(lightColor * diffuse * clamp(dot(lightDir, worldNormal), 0.0, 1.0), 1.0);
}
