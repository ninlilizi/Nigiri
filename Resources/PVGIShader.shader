...
inline float3 ConeTrace(float3 worldPosition, float3 coneDirection, float2 uv, float3 blueNoise)
{
	...
	int StochasticSampling = 1;
	int SEGISphericalSkylight = 1;
	...
	float3 currentPosition = coneOrigin + blueNoise * StochasticSampling;
	...
	for (float i1 = 0.0f; i1 < iteration1; i1 += 1.0f)
	{
		...
		if (hitFound < 0.9f)
		{
			currentVoxelInfo = GetVoxelInfo1(GetVoxelPosition(currentPosition)) * length;
			if (currentVoxelInfo.a > 0.0f)
			{
				hitFound = 1.0f;

				voxelBufferCoord = GetVoxelPosition(currentPosition) * length;
			}
		}
		float fi = ((float)i1) / iteration1;
		fi = lerp(fi, 1.0, 0.0);

		float coneDistance = GetVoxelPosition(currentPosition).z;

		float coneSize = coneDistance * width;
	...
	{
		currentPosition += (coneStep * coneDirection) + blueNoise * StochasticSampling;

		if (hitFound < 0.9f)
		{
			currentVoxelInfo = GetVoxelInfo2(GetVoxelPosition(currentPosition)) * length;
			if (currentVoxelInfo.a > 0.0f)
			{
				hitFound = 1.0f;
				voxelBufferCoord = GetVoxelPosition(currentPosition) * length;
	...
	for (float i3 = 0.0f; i3 < iteration3; i3 += 1.0f)
	{
		...
		if (hitFound < 0.9f)
		{
			currentVoxelInfo = GetVoxelInfo3(GetVoxelPosition(currentPosition)) * length;
			if (currentVoxelInfo.a > 0.0f)
			{
				hitFound = 1.0f;
	...
	for (float i4 = 0.0f; i4 < iteration4; i4 += 1.0f)
	{
		...
		if (hitFound < 0.9f)
		{
			currentVoxelInfo = GetVoxelInfo4(GetVoxelPosition(currentPosition)) * length;
			if (currentVoxelInfo.a > 0.0f)
			{
				hitFound = 1.0f;
				voxelBufferCoord = GetVoxelPosition(currentPosition) * length;
			}
			float fi = ((float)i4) / iteration4;
			fi = lerp(fi, 1.0, 0.0);
	...
	for (float i5 = 0.0f; i5 < iteration5; i5 += 1.0f)
	{
		...
		if (hitFound < 0.9f)
		{
			currentVoxelInfo = GetVoxelInfo5(GetVoxelPosition(currentPosition)) * length;
			if (currentVoxelInfo.a > 0.0f)
			{
				hitFound = 1.0f;
				voxelBufferCoord = GetVoxelPosition(currentPosition) * length;
	...
	//gi.rgb /= maximumIterations * 0.1;

	//Calculate lighting attribution
	float3 gnormal = normalize(tex2D(_CameraGBufferTexture2, uv).rgb * 2.0 - 1.0);
	float NdotL = pow(saturate(dot(gnormal, coneDirection) * 1.0 - 0.0), 0.5);
	...
	voxelBufferCoord.x += blueNoise.x;
	voxelBufferCoord.y += blueNoise.y;
	voxelBufferCoord.z += blueNoise.z;
	double index = voxelBufferCoord.x * (256) * (256) + voxelBufferCoord.y * (256) + voxelBufferCoord.z;
	tracedBuffer1[index] += float4(gi);

	float4 cachedResult = float4(tracedBuffer0[index]) / 32768;
...
inline float3 ComputeIndirectContribution(float3 worldPosition, float3 worldNormal, float2 uv)
{
	...
	float3 randomVector = normalize(blueNoise);

	float3 direction = normalize(cross(worldNormal, randomVector));

	float3 coneDirection = lerp(direction, worldNormal, 0.3333f);

	accumulatedColor += ConeTrace(worldPosition, coneDirection, uv, blueNoise);
...
float4 frag_lighting(v2f i) : SV_Target
{
	...
	float3 indirectLighting = ((ao * indirectLightingStrength * (1.0f - metallic) * albedo) / PI) * ComputeIndirectContribution(worldPos, worldSpaceNormal, i.uv);

	//float3 finalLighting = indirectLighting + lerp(directLighting.rgb, albedo * 0.25, 0.25);
