...
float3 DoReflectiveShadowMapping(float3 P, bool divideByW, float3 N, float4 uv, float4 tangent, Interpolators interpolator)
{
	...
	for (uint i = 0; i < rsmSampleCount; ++i)
	{
		...
		float3 vplNormalWS = N;
		float3 RSMflux = tex2D(_MainTex, coords.xy).xyz * CreateLight(interpolator).color;
...
FragmentOutput MyFragmentProgram (Interpolators i) {
	...
	float3 shadowColor = float3(0, 0, 0);// color.rgb;// *lightColor1;// *NL;/// *atten;
	///
		...
		if (emission.r > 0.1 || emission.g > 0.1 || emission.b > 0.1) output.gBuffer1.rgb = color.rgb + emission + lightColor + flux.rgb;
		///
		...
		#endif
	#else
		output.color = color + emission + lightColor;
	#endif
	return output;
}
